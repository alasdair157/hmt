...
from hmt.core import Forest, Node, Tree
from hmt.exceptions import HMTError
from hmt.utils import digamma, div0, mvn_pdf, normal_round, rowwise_outer
...
class HMModel:
    ...
    def init_params(self):
        ...
        if self.P is None:
            ...
            if self.sister_dep:
                self.P = np.random.uniform(
                    0, 1, (self.n_hidden, self.n_hidden, self.n_hidden)
                )
                # Normalise for each value of i
                self.P = (self.P.T / np.sum(self.P, axis=(1, 2))).T
                # self.P = np.full(
                #     (self.n_hidden, self.n_hidden, self.n_hidden),
                #     1 / self.n_hidden**2,
                # )
        ...
        if self.sigmas is None:
            # Initialise sigma based on variance in the data
            # NOTE: useless if normalising first
            var = self.var()
    ...
    def check_params(
        self, init_s_distr=None, P=None, emit_mat=None, mus=None, sigmas=None
    ):
        ...
        if P is not None:
            if P.shape[0] != self.n_hidden:
                raise HMTError(
                    "P must be rxr or rxrxr where r is the number of hidden states."
                )
            if P.ndim == 2:
        ...
        if mus is not None:
            if mus.shape[0] != self.n_hidden:
                raise HMTError(
                    "mu must have shape rxn where r is the number of hidden states and"
        ...
        if sigmas is not None:
            if sigmas.shape[0] != self.n_hidden:
                raise HMTError(
                    "Emission covaraince matrix must have shape rxnxn where r is the"
                    " number of hidden states and n is the number of observed states"
                )
            ...
            # if sigmas.ndim != 3:
            #     raise HMTError(
            #         "Emission covaraince tensor must be 3d, i.e. r 2d covariance"
            #         " matrices where r is the number of hidden states"
            #     )
...
class HMNode(Node):
    """A node in a hidden Markov tree. The methods impement the E step recursively via
    the upward-downward algorithm given in [1].
    ...
    def viterbi(self, urec=False):
        ...
        self.m_d_delta = np.amax(self.tree.P.T * delta, axis=0)
        return delta
    ...
    def sd_upward_pass(self, urec=False):
        ...
        if self.mother is None:
            self.tree.loglikelihood += np.log(
                np.dot(self.tree.emit(self.x), self.s_distr * self.beta_c_u)
            )
    ...
    def sample(self, N, p):
        ...
        if self.tree.sister_dep:
            ...
            s0, s1 = np.unravel_index(
                np.random.choice(
                    np.arange(self.tree.n_hidden**2), 1, p=c_distr.flatten()
                )[0],
                c_distr.shape,
            )
        else:
            curr_distr = self.tree.P[self.s]
            s0, s1 = np.random.choice(
                np.arange(self.tree.n_hidden), 2, p=curr_distr, replace=True
            )

        if N0 != 0:
            x0 = np.squeeze(
                np.random.multivariate_normal(
                    self.tree.mus[s0], self.tree.sigmas[s0], 1
                )
            )
        ...
        if N1 != 0:
            x1 = np.squeeze(
                np.random.multivariate_normal(
                    self.tree.mus[s1], self.tree.sigmas[s1], 1
                )
            )
            if self.tree.n_obs == 1:
                x1 = float(x1)
...
class HMTree(Tree, HMModel):
    ...
    def __init__(
        self, n_hidden, n_obs, sister_dep=True, daughter_order=False, root=None
    ):
        super().__init__(root)
    ...
    def emit(self, x):
        ...
        try:
            ...
            raise HMTError(
                "x contains null values and marginalised sigma matrix is singular."
            )
        marg_detsigma = np.linalg.det(marg_sigmas)
        return mvn_pdf(
            marg_x, marg_mus, marg_sigmainv, marg_detsigma, non_null_indices.shape[0]
        )
    ...
    def Mstep(self):
        ...
        xi_sum = self.sum("xi")
        leaf_sum = np.sum([leaf.xi for leaf in self.leaves], axis=0)
        ...
        ## Update mu
        self.mus = (
            self.sum(("xi", "x"), func=lambda xi, x: np.outer(xi, x)).T / xi_sum
        ).T
    ...
    def vb_init_hyperparams(self):
        ...
        # self.prior_sigma_Winv = np.stack(
        #     [np.eye(self.n_obs) for _ in range(self.n_hidden)]
        # )

        sample_cov = self.cov()
        if not isinstance(sample_cov, np.ndarray):
            prior_W = np.array([sample_cov for _ in range(self.n_hidden)]).reshape(
                -1, 1, 1
            )
        else:
    ...
    def vb_parameter_update(self):
        ...
        self.init_s_distr = np.exp(
            digamma(self.init_s_weights) - digamma(self.init_s_weights.sum())
        )
    ...
    def train(
        self, tol, maxits, store_log_lks=False, permute=True, overwrite_params=False
    ):
        if overwrite_params:
            self.clear_params()
        self.init_params()
...
class HMForest(Forest, HMModel):
    ...
    def set_params(
        ...
        **unused,
        ...
        super().set_params(
            init_s_distr, P, emit_mat, mus, sigmas, sigmainv, detsigma, **unused
        )
        self.update_trees()
    ...
    def init_params(self, training="ensemble"):
        ...
        if training == "individual":
            ...
            return
        raise HMTError(
            "Invalid training option, options are 'ensemble' and 'individual'"
        )
    ...
    def Mstep(self):
        ...
        leaf_sum = 0
        for tree in self.trees:
            leaf_sum += np.sum([leaf.xi for leaf in tree.leaves], axis=0)

        if self.P.ndim == 2:
            m_d_xi_sum = self.sum("m_d_xi")
            m_d_xi_sum = np.sum([tree.sum("m_d_xi") for tree in self.trees], axis=0)
            mother_xi_sum = np.sum(
                [tree.root.mother_xi_sum() for tree in self.trees], axis=0
            )
            self.P = (m_d_xi_sum.T / mother_xi_sum).T
            if not np.allclose(np.sum(self.P, axis=1), 1):
                print(np.sum(self.P, axis=1))
    ...
    def sample_corr(self, measure="true_s"):
        ...
        for i in range(self.n_hidden):
            ...
            # Sample standard deviation
            d0_std = np.sqrt(
                np.dot(si[:, 1] - d0_mean, si[:, 1] - d0_mean) / (n[i] - 1)
            )
            d1_std = np.sqrt(
                np.dot(si[:, 2] - d1_mean, si[:, 2] - d1_mean) / (n[i] - 1)
            )
            # print(d0_std, d1_std)
    ...
    def sample(self, n_trees, n_nodes, dropout):
        ...
        for i in range(n_trees - 1):
            tree = HMTree(
                self.n_hidden, self.n_obs, self.sister_dep, self.daughter_order
            )
