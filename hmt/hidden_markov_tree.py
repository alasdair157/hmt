...
class HMNode(Node):
    ...
    def __init__(self, node_id, observed, tree=None, s=None):
        ...
        self.beta = None
        self.m_d_beta = None # Mother-daughter beta 
        self.m_d_delta = None
    ...
    def calculate_s_distr(self, drec=False):
        ...
        if self.mother is not None:
            ...
            self.s_distr = self.mother.s_distr @ self.tree.P
        
        if not np.isclose(sum(self.s_distr), 1):
    ...
    def m_d_sum(self):
        if self.mother is None:
            ...
            total = self.m_d_xi
        if self.d0 is not None:
            total += self.d0.m_d_sum()
        if self.d1 is not None:
            total += self.d1.m_d_sum()
        return total
...
class HMTree(Tree):
    ...
    def Mstep(self):
        ...
        if self.P.ndim == 2:
            ...
            # print(m_d_xi_sum)
            self.P = (m_d_xi_sum.T / (xi_sum - leaf_sum)).T / 2 #TODO WHY / 2 ??
    ...
    def check_model_params(self, init_s_distr=None, P=None, emit_mat=None, mus=None, sigmas=None):
    ...
    def set_model_params(self, init_s_distr=None, P=None, emit_mat=None, mus=None,
                         sigmas=None, sigmainv=None, detsigma=None, check_params=True, **unused):
        if check_params:
            self.check_model_params(init_s_distr, P, emit_mat, mus, sigmas)
...
class HMForest(Forest):
    ...
    def update_trees(self):
        for tree in self.trees:
            tree.set_model_params(**self.__dict__)
    

    def init_params(self, training='ensemble'):
        if training == 'ensemble':
            # Initialise parameters on one tree
            self.trees[0].init_params()
            # Steal these parameters and set all other trees to the same
            self.init_s_distr = self.trees[0].init_s_distr
            self.P = self.trees[0].P
            self.emit_mat = self.trees[0].emit_mat
            self.mus = self.trees[0].mus
            self.sigmas = self.trees[0].sigmas
            self.sigmainv = self.trees[0].sigmainv
            self.detsigma = self.trees[0].detsigma
            self.update_trees()
            return
    ...
    def EMstep(self, training='ensemble'):
        ...
        if training == 'individual':
            loglikelihood = 0
            for tree in self.trees:
                loglikelihood += tree.EMstep()
            return loglikelihood
        raise HMTError("Invalid training option, options are 'ensemble' and 'individual'")


    def train(self, tol, maxits, training='ensemble', store_log_lks=False):
